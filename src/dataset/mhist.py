...
def get_annotations(partition: str) -> pd.DataFrame:
    ...
    map_targets = {
        ...
        for target, encoded_target in zip(sorted(possible_targets), range(len(possible_targets)))
    }
    df["target"] = df["target_label"].map(map_targets)

    df["image_path"] = df.apply(
        lambda r: MHIST_IMAGE_PATH.format(image_name=r["image_name"]),
        axis=1
    )
